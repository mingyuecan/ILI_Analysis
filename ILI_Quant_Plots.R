...
draw_themes <- function(t) {
  ggplot(data = new, aes(new[t])) + geom_histogram(binwidth = 5,
                                                   col = "black",
                                                   fill = "lightskyblue2") + labs(title = t, x = "", y = "Count") + scale_x_continuous(breaks = seq(0, 100, 10)) + 
    theme(plot.title = element_text(hjust = 0.5))+
...
draw_subtopics <- function(s) {
  ...
  ggplot(data = new, aes(new[s])) + geom_histogram(binwidth = 0.5,
                                                   col = "black",
                                                   fill = "lightskyblue2") + labs(title = p, x = "", y = "Count") + scale_x_continuous(breaks = seq(0, 10, 1)) +
    theme(plot.title = element_text(hjust = 0.5))+
